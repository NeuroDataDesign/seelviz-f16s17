...
def submitJob(bucket, data, public_access_key, secret_access_key, myJobName):
    ...
    response = batch.submit_job(jobName=myJobName, jobQueue=myJobQueue, jobDefinition=myJobDefinition,
                                containerOverrides={'command': myCommandOverride, 'environment': myEnvironmentOverride})
    return response
